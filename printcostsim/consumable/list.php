...
$contextpage = GETPOST('contextpage', 'aZ') ? GETPOST('contextpage', 'aZ') : 'consumablelist';
...
if (empty($reshook)) {
    ...
    // Mass actions
    $objectclass = 'Consumable';
    $objectlabel = 'Consumable';
...
$now = dol_now();
$help_url = '';
$title = $langs->trans("Consommables");
$morejs = array();
$morecss = array();
...
if ($num == 1 && !empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all && !$page) {
    ...
    $id = $obj->rowid;
    header("Location: ".dol_buildpath('/printcostsim/consumable/card.php', 1).'?id='.$id);
    exit;
}
...
$newcardbutton = '';
if ($permissiontoadd) {
    $newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/printcostsim/consumable/card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);
}
